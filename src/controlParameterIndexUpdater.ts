...
export class ControlParameterIndexUpdater {
  ...
  handleMouseMove(event: MouseEvent) {
    ...
    window.__JUCE__.backend.emitEvent(
      JuceEvents.ControlParamaterIndexChanged,
      controlParameterIndex
    );
  }

  //==============================================================================
  private getControlParameterIndex(element: Element) {
    const isValidNonRootElement = (e: Element) => {
    ...
    while (isValidNonRootElement(element)) {
      if (element.hasAttribute(this.controlParameterIndexAnnotation)) {
        return Number(
          element.getAttribute(this.controlParameterIndexAnnotation)
        );
      }
